...
// and log the result
let allCaps = [];
const firstNames = runners.map(function(runner) {
    return runner.first_name.toUpperCase();
})
allCaps.push(firstNames);
console.log(allCaps); 
...
let largeShirts = [];
const shirtSizes = runners.filter(function(runner) {
    // return runner.shirt_size;
    // console.log(runner.shirt_size)
    return runner.shirt_size === "L";
})
largeShirts.push(shirtSizes);
...
// and the 5k fun run theme.  Try to create and then solve 3 unique problems using
